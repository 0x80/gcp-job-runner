...
- **Unified file I/O** — `getFileWriter()` writes JSON/CSV/binary and `getFilesPath()` exposes the destination for reads, targeting a local directory during development and a Cloud Storage bucket in production with the same handler code
